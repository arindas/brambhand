...
- R8.05 replay-only ingest baseline:
  ...
  - added reusable CLI schema helpers in `client/common/cli_parse.*` for required option sets, mutually-exclusive groups, and typed value transforms (`cli_assign_transformed_value`), then applied them to desktop and benchmark option parsing paths
  - extended live runtime-bridge schema parity with replay body lifecycle metadata by adding `SimulationFrame.body_id_catalog` (`BodyIdCatalog`) to `interfaces/runtime_bridge.proto` and shipping Python stream publisher helpers (`brambhand.bridge.runtime_stream.RuntimeStreamPublisher`) that emit/validate replay-parity init+diff catalogs
