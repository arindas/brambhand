...
| FR-157..FR-160 | body-lifecycle registry contract tests (init/create/destroy ordering), replay body-id-catalog ingest tests (init+diff application), stream-schema parity tests for body-id catalogs, no-body-scan guard tests in consumers | replay/live parity scenarios proving body-ID lifecycle consistency across topology split/attach, spawn/despawn, and long-run catalog continuity |
...
- **R8.1 gate:** mission-control/onboard view-model schemas are versioned, validated, replay-compatible, and include replay/live body-id lifecycle parity (`body_id_catalog` init+diff semantics)
